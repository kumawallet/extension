export const styleSingIn = {
  pageWrapper: "bg-[#1F1432] h-[100dvh] relative !px-0 !py-0",
  innerWrapper: "py-6",
  containerBody: "flex flex-col px-4",
  logo: "mx-auto mt-[7rem] mb-4 w-[9rem] h-[9rem]",
  welcome: "font-medium text-2xl mb-2 text-center",
  description: "font-light text-sm mb-9 text-center",
  containerForm: "relative mb-2",
  input: "border-0  bg-[#1C1C27] h-[4rem] px-10 w-full",
  inputButton:
    "absolute right-3 top-[35px] -translate-y-1/2 cursor-pointer z-50",
  iconEye: "cursor-pointer",
  iconKey: "absolute top-[25px] left-3 w-7 h-4",
  button:
    "font-light text-base max-w-md h-[4rem] w-full py-2 md:py-4 mx-auto z-10 !bg-[#040404] !text-white",
  forgotPass: "text-center mt-4 mb-6 z-10 cursor-pointer",
};
