...
export const AccountDetails: FC<AccountDetailsProps> = ({
  ...
  accountData,
    ...
    const { 
              state: { accounts }, updateAccountName, deleteAccount
          } = useAccountContext();
    const {state: { selectedChain }} = useNetworkContext();


    const { Icon, copyToClipboard } = useCopyToClipboard(accountData.value.address);
  ...
  const getLink = () => {
      ...
      const typeNet = network.length > 3 ? network.split('').splice(-4).join('') : network
  ...
  const validatePassword = async () => {
    try {
      ...
      let privateKeyOrSeed: string | undefined = await messageAPI?.validatePassword({
        password,
        key,
        keyring
      });
    ...
    <div className="flex flex-col mt-10 gap-y-4">
        <div className="flex justify-between items-center">
            ...
            <button onClick={() => setIsOpen(true)}>
              <RiDeleteBinLine className="text-base" color="#FF0202" height="20px"/>
            </button>
          </div>
          <ConfirmMessage 
                  isOpen={isOpen} 
          ...
          <div >
              ...
              <div className="flex">
                <p className="opacity-80">
                  {getHash(accountData?.value.address)}
                </p>
          ...
          <div>
              { accountData.value.keyring === "EVM" ?
                (<p>{t("private_key")}</p>) :
                (<p>{t("seed_phrase")}</p>) 
              } 
              <div className="relative my-8">
                  <textarea
                          className="p-2 text-center text-xs h-[100px]  focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 bg-[#1C1C27]  placeholder-gray-400 text-white resize-none relative select-none"
                          value={privateKey}
                          ...
                          onFocus={handleFocus}
                  />
                  <div className={`absolute left-0 top-0 w-full h-full ${hideKeys ? "backdrop-blur-sm"  : !hideKeys && !validated ? "bg-[#171720]" : ""}   flex justify-center items-center`}> 
                        {
                          hideKeys ? 
                              (
                              <button
                              ...
                              <div className="flex flex-col w-full border-3 border-white gap-y-4 mt-[2rem]">
                                  <div className="relative mb-2 border-3 border-white">
                                        <input
                                          ...
                                          onKeyDown={({ key }) => key === "Enter" && validatePassword()}
                                        />
