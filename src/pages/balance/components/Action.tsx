...
export const Action: FC<ActionProps> = ({ Icon, title, onClick, isDisabled = false }) => {
  ...
  return (
    <button className={`group min-w-14 bg-[#0E0D0D] relative rounded-xl px-2 pb-3 pt-4 flex flex-col items-center overflow-hidden hover:enabled:bg-${color}-fill`} disabled={isDisabled} onClick={!isDisabled ? onClick : undefined}>
