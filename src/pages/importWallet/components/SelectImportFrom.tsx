  ...
  return (
    <>
      <div className="flex flex-col gap-7 mt-10 md:mt-20 mb-8">
        <Button classname="w-full py-6 text-start text-base md:text-2xl" isDisabled={typeToImport === "evm"} variant="contained-black" onClick={() => onSelected('seed')}>{t("import_from_seed_phrase")}</Button>
        <Button classname="w-full py-6 text-start text-base md:text-2xl" isDisabled={typeToImport === "wasm"} variant="contained-black" onClick={() => onSelected('privateKey')}>{t('import_from_private_key')}</Button>
      </div>
