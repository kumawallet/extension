...
import { isKnownEstimatedFeeError, validateRecipientAddress } from "@src/utils/transfer";

interface FeeAndTipProps {
  containerClassname?: string
}

export const FeeAndTip: FC<FeeAndTipProps> = ({
  containerClassname
}) => {
  ...
  useEffect(() => {
    ...
    if (!validateRecipientAddress(recipient, originNetwork.type as "evm" | "wasm")) {
      return
    }
      ...
      try {
        ...
        if (originChainType === "wasm") {
          ...
          if (isXCM) {
            ...
            const contract = new Contract(contractAddress, abi as string, signer as Wallet);
            ...
            const estimatedFee = feeData.maxFeePerGas?.mul(gasLimit).add(feeData.maxPriorityFeePerGas!) || "0";
            ...
            setValue("evmTx", {
              ...
              maxPriorityFeePerGas: feeData.maxPriorityFeePerGas as unknown as BigNumberish,
              type: 2,
            })
            ...
            const result = await contract.populateTransaction.transfer(partialTx.to, partialTx.value, {
              gasLimit,
              maxFeePerGas: feeData.maxFeePerGas as unknown as BigNumberish,
              maxPriorityFeePerGas: feeData.maxPriorityFeePerGas as unknown as BigNumberish,
            })
  ...
  }, [asset, originNetwork, targetNetwork, recipient, api, amount]);
  ...
  return (
    <div className={`${containerClassname} flex flex-col gap-2 justify-around py-3 px-4 bg-[#1C1C27] rounded-xl`}>
      ...
      <div className="w-full flex justify-between items-center">
        ...
        <Switch
          ...
          className={`${isTipEnabled ? 'bg-teal-400' : 'bg-gray-400'
            } relative inline-flex h-[0.6rem] w-[1.9rem] items-center rounded-full`}
        >
          ...
          <div className="py-1">
            <div className="flex items-center p-2 border border-[#636669] rounded-2xl">
              <NumericFormat
                ...
                thousandSeparator=","
                onInput={(e) => {
                  e.currentTarget.style.width = (e.currentTarget.value.length || 1) + 1 + "ch";
                }}
              />
