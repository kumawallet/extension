...
export default class AssetsBalance  {
 ...
 public async setAssets(account:  AccountEntity, api : api,chain : string){
    try{
    ...
    if(api.type === "evm" && this.getType(account.type.toLowerCase()) === "evm"){
      const { nativeAssets, unsub }  = await this.getNativeAsset(api,account,_chain)
      const {_assets, _unsubs} = await this.getOtherAssets(api,account,_chain)
        const subs = _unsubs || []
        const assets = this.assets.getValue()
      const network : any = !assets[account.key] || Object.keys(assets[account.key]).length === 0 ? {} : {...assets[account.key]}
        network[chain] = {
      ...
      const { nativeAssets, unsubs }  = await this.getNativeAsset(api,account,_chain)
      const {_assets, _unsubs} = await this.getOtherAssets(api,account,_chain)
        const subs = _unsubs || []
        const assets = this.assets.getValue()
        const network : any = !assets[account.key] || Object.keys(assets[account.key]).length === 0 ? {} : {...assets[account.key]}
 ...
 private getNativeAsset = async (
    ...
    if(allAsset[account][chain]){
      const index = allAsset[account][chain].assets.findIndex((asset) => {
        ...
        if(index > -1 && !data.balance?.eq(allAsset[account][chain].assets[index].balance)){
            const _balance = Number(
                formatAmountWithDecimals(Number(data.balance), 6, allAsset[account][chain].assets[index]?.decimals)
              );
              if(data?.frozen && data.reserved && data.transferable){
                allAsset[account][chain].assets[index] = {
                    ...allAsset[account][chain].assets[index],
                    amount: Number(((allAsset[account][chain].assets[index].price || 0) * _balance).toFixed(2)),
