...
export default class HydraDx {
  ...
  public async getFee(
    ...
    try {
      const _sellResult = await this.tradeRouter?.getBestSell(
        assetToSell.id,
        assetToBuy.id,
        amount
      );
      ...
      const minReceive = _sellResult?.amountOut
        .times(1 - slippage)
        .integerValue();
  ...
  public async getassetsBuy(asset: SwapAsset) {
    try {
      ...
      const assets =
        (await this.tradeRouter?.getAssetPairs(asset.chainId)) || [];
      ...
      assets.forEach((assetBuy) => {
        ...
        if (find) {
          const _assetBuy = {
            ...
            image:
              ASSETS_ICONS[
                assetBuy.symbol === "4-Pool"
                  ? "FOURPOOL"
                  : assetBuy.symbol === "2-Pool"
                  ? "TWOPOOL"
                  : assetBuy.symbol
              ],
            balance: "0",
