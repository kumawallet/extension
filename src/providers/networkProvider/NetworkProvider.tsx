...
export const getProviderforNetworks = async (networks: Chain[], api: any) => {
  ...
  const provider: {
    [id: string]: ApiPromise | ethers.providers.JsonRpcProvider | null;
  } = {};
  ...
  if (Object.keys(api).length === 0 || !api) {
    networks.forEach((network) => {
      if (network.type === "evm") {
        provider[network.id] = new ethers.providers.JsonRpcProvider(
          network.rpcs[0] as string
        );
      }
      if (network.type === "wasm") {
        const promise = ApiPromise.create({
          provider: new WsProvider(network.rpcs),
        });
    ...
    try {
      if (Object.prototype.hasOwnProperty.call(state.selectedChain, id)) {
        ...
        if (state.api[id] && "getBalance" in state.api[id]) {
          (
            state.api[id] as ethers.providers.JsonRpcProvider
          ).removeAllListeners("block");
        }
    ...
    const setProvider = async () => {
      try {
        const Chains = allChains.filter((chain) =>
          Object.keys(state.selectedChain).includes(chain.id)
        );
